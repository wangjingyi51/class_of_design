...
Delete_path::Delete_path(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::Delete_path)
{
    ui->setupUi(this);
    //配置主窗口
    this->setFixedSize(1500,1000);
    this->setWindowTitle("管理员删除路径");
    ...
    //地点选择
    QFile file("D:\\ClassDesign_picture\\school_route.TXT");
    ...
    while(!file.atEnd())
    {
        ...
        QString location1 = list[1];
        ui->comboBox->addItem(location);
        ui->comboBox_2->addItem(location1);
    }
    //从文件中删除路径（先全读出来，然后删除以后再写回去）
    connect(ui->pushButton,&QPushButton::clicked,[=]()
    {
        ...
        QString data1 = ui->comboBox_2->currentText();
        ...
        {
            if(list3[i].contains(data,Qt::CaseSensitive)&&list3[i].contains(data1,Qt::CaseSensitive))
            {
                string = list3[i];
                break;
            }
        ...
        for(int k=0;k<list3.size();k++)
        {
            ...
            if(k<list3.size()-1)
            {
                file.write(list3[k].toStdString().c_str());
                file.write("\n");
            }
            else
            {
...
void Delete_path::paintEvent(QPaintEvent *)
{
    //校园导航底图

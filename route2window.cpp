...
#include "ui_route2window.h"
#include "graph.h"
#include "QFile"
#include "QPainter"
#include "QDebug"

route2window::route2window(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::route2window)
{
    ui->setupUi(this);
    //配置主窗口
    ...
    connect(ui->pushButton,&QPushButton::clicked,[=](){
            emit this->Route2Back();
    });
//    update();
}
void route2window::Iq_shortestroute(QString start, QString end)
{
    ...
    //graph.print_graph();
    QString a;
    a = graph.Dijkstra(start,end);
    this->Route = a;
    QStringList list = a.split(" ");
    ui->textBrowser->insertPlainText("最短路径：");
    ui->textBrowser->append(list[0]);
    ui->textBrowser->append("路径长度：");
    ui->textBrowser->append(list[1]);


}
void route2window::paintEvent(QPaintEvent *)
{
    //校园导航底图
    ...
    // 设置画笔颜色、宽度
    QStringList list2 = this->Route.split(" ");
    QString luxian = list2[0];
    QStringList list3 = luxian.split("->");
    qDebug()<<list3;
